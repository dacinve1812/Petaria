/**
 * Migration: users.guess_number_* (Đoán số / Game Center).
 *
 * Tương đương `db/migrations/20260508_guess_number_daily.sql`, chạy bằng Node + mysql2.
 * Idempotent: chạy lại sẽ bỏ qua cột đã tồn tại.
 *
 * Usage (từ thư mục petaria):
 *   node scripts/migrate_guess_number_daily.js --dry-run
 *   node scripts/migrate_guess_number_daily.js
 *
 * npm:
 *   npm run migrate:guess-number
 *
...
async function run() {
  ...
  try {
    const steps = [
      {
        column: 'guess_number_period_key',
        sql: `ALTER TABLE users
          ADD COLUMN guess_number_period_key VARCHAR(64) NULL DEFAULT NULL
          COMMENT 'Kỳ đếm lượt (epoch ms string, theo global_reset_time)'`,
      },
      {
        column: 'guess_number_daily_plays',
        sql: `ALTER TABLE users
          ADD COLUMN guess_number_daily_plays INT NOT NULL DEFAULT 0
          COMMENT 'Số lần đoán đã hoàn thành trong kỳ'`,
      },
      {
        column: 'guess_number_pending_json',
        sql: `ALTER TABLE users
          ADD COLUMN guess_number_pending_json TEXT NULL
          COMMENT 'Vòng đang chơi JSON {secret,pivot,minS,maxS}'`,
      },
    ];
    ...
    console.log(dryRun ? 'Dry-run xong.' : 'Migrate guess number daily: hoàn tất.');
  } finally {
    await conn.end();
  }
}
