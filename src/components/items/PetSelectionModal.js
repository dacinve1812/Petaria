...
function PetSelectionModal({
  ...
  const getActionText = () => {
    switch (action) {
      ...
      case 'heal':
        return 'Chữa trị';
      case 'restore_energy':
        return 'Hồi phục năng lượng';
      case 'boost_stats':
        return 'Gia tăng chỉ số';
      case 'play':
        return 'Chơi đùa';
      case 'transform':
        return 'Thay đổi hình dạng';
      case 'sell_auction':
        return 'Đặt vào cửa hàng';
      case 'exhibition':
        return 'Mang vào phòng triển lãm';
      case 'gift':
        return 'Tặng cho bạn bè';
      default:
        return 'Sử dụng';
    }
  };
  ...
  const getActionDescription = () => {
    switch (action) {
      case 'equip':
        return 'Chọn thú cưng để trang bị:';
      case 'sell':
        return 'Xác nhận bán vật phẩm:';
      case 'feed':
        return 'Chọn thú cưng để cho ăn:';
      case 'heal':
        return 'Chọn thú cưng để chữa trị:';
      case 'restore_energy':
        return 'Chọn thú cưng để hồi phục năng lượng:';
      case 'boost_stats':
        return 'Chọn thú cưng để gia tăng chỉ số:';
      case 'play':
        return 'Chọn thú cưng để chơi đùa:';
      case 'transform':
        return 'Chọn thú cưng để thay đổi hình dạng:';
      case 'sell_auction':
        return 'Đặt vật phẩm vào cửa hàng:';
      case 'exhibition':
        return 'Mang vật phẩm vào phòng triển lãm:';
      case 'gift':
        return 'Chọn thú cưng để tặng vật phẩm:';
      default:
        return 'Chọn thú cưng để sử dụng:';
    }
  };
  ...
  return (
    ...
    >
      <div className="pet-selection-modal-instruction">
        {/* <p>
          Select the <span className="highlight">number of items</span> to{' '}
          {getActionText().toLowerCase()}.
        </p> */}
        <p>
          {getActionText()} {quantity} {item?.name || 'item'}(s).
        </p>
      </div>

      {requiresPetSelection() && (
        <div className="pet-selection-modal-section">
          <label className="pet-selection-modal-label">{getActionDescription()}</label>
          <select
