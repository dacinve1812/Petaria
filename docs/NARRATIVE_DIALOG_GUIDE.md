...
| Ví dụ dùng | `BeggarKingGame.js`, `DailyFreeItemsGame.js`, `GuessNumberGame.js`, `SlotMachineGame.js` (ẩn/hiện UI) |
| Config admin | Game center → tab tương ứng | `*.narrative` trong JSON config |
...
| Dialog đè mất UI chơi (guess-number…) | Dùng **ẩn/hiện**: `storyPhase` `intro` → `play` (ẩn narrative, hiện board) → `result`. Tham khảo `GuessNumberGame.js` |

---

## 7. Pattern ẩn/hiện (trang có UI tương tác)

Khi page **đã có board / nút chơi** (vd. Đoán số — Làng Trẻ Con):

1. Intro: chỉ render `NarrativeScene` (cùng CSS như beggar-king / daily-free).
2. Hết thoại / Skip / nút chơi → **ẩn** narrative, **hiện** UI chơi.
3. Sau kết quả vòng → ẩn board, hiện lại narrative (win/lose) → Continue → hiện board lại.

Không overlay đè lên nội dung — đổi phase bằng state. Config: `guessNumber.narrative` / `slotMachine.narrative` (Admin → tab tương ứng).

---

**Cập nhật:** July 2026 — beggar-king, daily-free, guess-number, slot-machine (ẩn/hiện).
