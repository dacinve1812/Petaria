...
| location_id | `0` = Arena; `≥1` = map săn (map id tương ứng); `NULL` = legacy (vẫn hiện Arena) |
...
Sau khi chạy, thêm dữ liệu Boss và Skill qua admin hoặc SQL. Arena sẽ hiển thị Boss có `location_id = 0` hoặc `location_id IS NULL`. Boss map săn dùng `location_id` = id map (≥1), base L1 + công thức khi encounter.
...
- **GET /api/arena/enemies**  
  Trả về danh sách Boss (id, name, level, image, isBoss: true) từ `boss_templates` với `location_id = 0 OR location_id IS NULL`.
...
- **Stat Boss Arena** (`location_id = 0`): `base_*` dùng **trực tiếp** làm combat stats — không công thức, không IV.
- **Stat Boss map săn** (`location_id ≥ 1`): `base_*` = base L1; khi encounter truyền `bossLevel` / `?level=` → tính bằng công thức pet (IV 31).
